...
stripe.api_key = 'sk_test_O1llb1lazTGfzCRG0RA2lzMu00znEGIiQi'
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['plan']== 'yearly':
            plan = 'prod_FQScPwnyzLqB2Y' #plan id in stripe for monthly

        if request.POST['plan']== 'yearly':
            plan = 'plan_FQSflkY2ZRx15A' #plan id in stripe for yearly
...
def settings(request):
    ...
    if request.method == 'POST':
        subscription = stripe.Subscription.retrieve(reqeust.user.customer.stripe_subscription_id)
